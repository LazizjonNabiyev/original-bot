...
def kb_menu(lang):
    if lang == "ru":
        return ReplyKeyboardMarkup([["📝 Жалоба"],["📍 Локация", "📞 Контакты"],["💼 Вакансии"]], resize_keyboard=True)
    return ReplyKeyboardMarkup([["📝 Shikoyat"],["📍 Lokatsiya", "📞 Bog'lanish"],["💼 Vakansiyalar"]], resize_keyboard=True)
...
async def handle_update(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if s.get("admin_mode"):
        ...
        if s.get("admin_step") == "vac_title":
            ...
            await msg.reply_text("📝 Tavsifini yozing (ish vaqti, maosh, talablar):")
            return
        ...
        if text.startswith("/delvac_"):
            try:
                ...
                await msg.reply_text(f"✅ Vakansiya #{vid} o'chirildi", reply_markup=kb_admin())
            except:
                await msg.reply_text("Xato")
            return
    ...
    if text == "🇺🇿 O'zbekcha":
        ...
        await msg.reply_text(f"🛒 Original Supermarket\n\nSalom, {name}! Quyidagilardan birini tanlang:", reply_markup=kb_menu("uz"))
        return

    if text == "🇷🇺 Русский":
        set_s(uid, {"step": "menu", "lang": "ru"})
        name = msg.from_user.first_name or "Друг"
        await msg.reply_text(f"🛒 Original Supermarket\n\nПривет, {name}! Выберите раздел:", reply_markup=kb_menu("ru"))
        return
    ...
    if text in ["📝 Shikoyat", "📝 Жалоба"]:
        ...
        await msg.reply_text("✏️ Shikoyatingizni yozing:" if lang=="uz" else "✏️ Напишите вашу жалобу:", reply_markup=kb_back(lang))
        return

    if text in ["📍 Lokatsiya", "📍 Локация"]:
        kb = InlineKeyboardMarkup([[InlineKeyboardButton("🗺 Google Maps", url=LOCATION_URL)]])
        await msg.reply_text("📍 *Original Supermarket*\n\nManzil: Qarshi, Qashqadaryo\nRQJM+9Q9, Qarshi", parse_mode="Markdown", reply_markup=kb)
        return
    ...
    if text in ["📞 Bog'lanish", "📞 Контакты"]:
        ...
        await msg.reply_text(f"📞 *Bog'lanish*\n\n📱 {PHONE}\n📸 @original_supermarket_\n💬 @LazizjonNabiyev", parse_mode="Markdown", reply_markup=kb)
        return

    if text in ["💼 Vakansiyalar", "💼 Вакансии"]:
        active = [(vid, v) for vid, v in vacancies.items() if v.get("active")]
        if not active:
            await msg.reply_text("Hozircha bo'sh ish o'rinlari yo'q 😔" if lang=="uz" else "Пока нет открытых вакансий 😔")
            return
        result = "💼 Bo'sh ish o'rinlari:\n\n" if lang=="uz" else "💼 Открытые вакансии:\n\n"
        for vid, v in active:
            result += f"🔹 *{v['title']}*\n{v['desc']}\n\n"
        result += f"📞 Murojaat: {PHONE}"
    ...
    if s.get("step") == "get_text":
        ...
        await msg.reply_text("📸 Rasm biriktirmoqchimisiz?" if lang=="uz" else "📸 Хотите фото?", reply_markup=kb_photo(lang))
        return

    if s.get("step") == "ask_photo":
        if text in ["📷 Ha, rasm", "📷 Да, фото"]:
            set_s(uid, {**s, "step": "get_photo"})
            await msg.reply_text("📸 Rasmni yuboring:" if lang=="uz" else "📸 Отправьте фото:", reply_markup=kb_skip(lang))
            return
        label = "📝 SHIKOYAT" if lang=="uz" else "📝 ЖАЛОБА"
        await send_to_channel(ctx.bot, label, s["text"], msg.from_user)
        del_s(uid)
        await msg.reply_text("✅ Qabul qilindi! Tez orada javob beramiz 🙏" if lang=="uz" else "✅ Принято! 🙏", reply_markup=kb_menu(lang))
        return
    ...
    if s.get("step") == "get_photo":
        ...
        del_s(uid)
        await msg.reply_text("✅ Qabul qilindi! 🙏" if lang=="uz" else "✅ Принято! 🙏", reply_markup=kb_menu(lang))
        return
